...
print("Welcome to pyOS 1.0.5 Beta.")
...
while True:
    ...
    if cmd == "exit":
        ...
        base_dir = os.path.dirname(os.path.abspath(__file__))
        clock_path = os.path.join(base_dir, "clock.py")
        if os.path.exists(clock_path):
            try:
                subprocess.run(["python", clock_path])
            except Exception as e:
                print(f"Error {e}!")
                print("error. Cannot open Clock.py or clock.py not in the same folder.")
                print("Please put clock.py in the same folder as this file.")
                print("If there is no clok file you can reinstall this OS in:")
                print("https://github.com/Z4RN-cmd/pyOS-my-version-")
        else:
            print("error. Cannot open Clock.py or clock.py not in the same folder.")
            print("Please put clock.py in the same folder as this file.")
